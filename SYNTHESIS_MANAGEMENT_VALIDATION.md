...
- 报告必须区分：UI 路径验证、表格导入异常验证、接口健壮性探测。
- V4.0.5 起主结论必须按“正常人在 UI 上能操作”的口径执行：UI 会拦截的负例只记录为前端校验，不得绕过 UI 强行提交后端并混入主结论；UI 不可选/不可填的伪造字段只作为单独 API 探测。
...
- 播报音频上传：只使用 UI 可选 MP3/WAV，覆盖大小、采样率、声道、bit depth、码率、损坏文件、空文件。
...
- 音频上传接口放行 WAV 8bit、WAV 24bit、MP3 64kbps，与页面限制提示不一致。
...
- 入口：播报合成版本配置中的“批量导入”，实际调用 `/biz/audiofile/batchImport`。
...
- 已验证并正确拒绝：文件 `>20KB`、采样率 `32000/48000`、双声道、码率 `40/64kbps`、损坏 mp3、0 字节 mp3、映射文件非 `.xlsx`、缺少映射文件。
- 当前后端会放行但按页面规则应拒绝：`.wav` 音频、mp3 内容改成 `.txt/.aac` 后缀、`64000Hz WAV`、xlsx 中 `音频描述` 与音频文件名不匹配、缺少/为空的 `播报内容/音频描述/接收协议`、非法 `接收协议`、只上传 xlsx 不上传音频。
- 下游复核：上述被放行的异常导入行继续调用 `/biz/broadcastrelease/add` 时也会创建成功，后端没有二次拦截。
