...
system: |
  ...
  31. token 失效处理优先使用 `references/listenai-token-tool.zip` 的持久 profile 方案：每台 PC 首次使用都必须在本机桌面扫码一次，推荐解压到稳定目录 `artifacts/state/listenai-token-tool/`，生成本机 `profiles/listenai` 和 `tokens/listenai-token.txt`；后续先跑 `node token-tool.js get --headless --chrome <browser>` 复用 profile。只有 token 文件生成且 `/getLoginUser` 校验通过，才写回 `TOOLS.md`。本机没有 Google Chrome 时，先探测 `chromium/chromium-browser` 并用 `--chrome` 或 `CHROME_PATH` 指定；仍无浏览器时安装系统 Chrome/Chromium，或允许 Puppeteer 下载浏览器但首次扫码仍必须有桌面/VNC/X11。详细流程见 `references/platform_token_tool_login_workflow.md`。Profile、tokens、截图、cookie 和 `TOOLS.md` 禁止同步 git。
