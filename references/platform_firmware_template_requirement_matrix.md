...
| `assets/templates/algo_zh_voice_reg_specific.xlsx` | zh | 语音注册指定学习：学习命令词/唤醒词、删除、退出、全部删除、可学习目标命令。 | voiceRegEnable=true；releaseRegist.registMode=specificLearn；repeat/retry 正反例 | 指定学习；学习命令词；学习唤醒词；删除命令词；删除唤醒词；退出学习/删除；全部删除 | 中：负例不能使用语音注册控制词，避免误触发真实功能。 |
| `assets/templates/algo_zh_voice_reg_continuous.xlsx` | zh | 语音注册连续学习：连续学习入口、直学/自动下一条、两步删除命令词、模板满后清理再学。 | releaseRegist.registMode=contLearn；连续学习状态机回归 | 连续学习；自动下一条；两步删除命令词；模板满处理；重试耗尽 | 中：学习态需等待提示播报结束和算法重建完成。 |
| `assets/templates/algo_zh_voice_reg_boundary_delete.xlsx` | zh | 语音注册边界/删除：字数上下限、重试次数、模板上限、删除命令/唤醒词/全部删除闭环。 | 语音注册边界包；删除闭环包；retryCount/repeatCount/maxLimit 边界 | 字数上下限；重试次数；模板上限；删除闭环；失败后不生效 | 中：字数边界语料运行时合成，模板只提供入口和宿主动作。 |
...
| `assets/templates/algo_zh_full_feature_stateful.xlsx` | zh | 综合状态包：基础功能 + 协议主动/被动 + 多唤醒指定切换 + 语音注册控制词。 | 全功能保持开启包；功能耦合冒烟；报告前综合回归 | 基础功能；协议；多唤醒；语音注册；掉电保持 | 高：词量较多，3021 上如出现内存超限，应拆分为专项模板而不是压缩结论。 |
...
| `assets/templates/algo_en_voice_reg_specific.xlsx` | en | 语音注册指定学习：学习命令词/唤醒词、删除、退出、全部删除、可学习目标命令。 | voiceRegEnable=true；releaseRegist.registMode=specificLearn；repeat/retry 正反例 | 指定学习；学习命令词；学习唤醒词；删除命令词；删除唤醒词；退出学习/删除；全部删除 | 中：负例不能使用语音注册控制词，避免误触发真实功能。 |
| `assets/templates/algo_en_voice_reg_continuous.xlsx` | en | 语音注册连续学习：连续学习入口、直学/自动下一条、两步删除命令词、模板满后清理再学。 | releaseRegist.registMode=contLearn；连续学习状态机回归 | 连续学习；自动下一条；两步删除命令词；模板满处理；重试耗尽 | 中：学习态需等待提示播报结束和算法重建完成。 |
| `assets/templates/algo_en_voice_reg_boundary_delete.xlsx` | en | 语音注册边界/删除：字数上下限、重试次数、模板上限、删除命令/唤醒词/全部删除闭环。 | 语音注册边界包；删除闭环包；retryCount/repeatCount/maxLimit 边界 | 字数上下限；重试次数；模板上限；删除闭环；失败后不生效 | 中：字数边界语料运行时合成，模板只提供入口和宿主动作。 |
...
| `assets/templates/algo_en_full_feature_stateful.xlsx` | en | 综合状态包：基础功能 + 协议主动/被动 + 多唤醒指定切换 + 语音注册控制词。 | 全功能保持开启包；功能耦合冒烟；报告前综合回归 | 基础功能；协议；多唤醒；语音注册；掉电保持 | 高：词量较多，3021 上如出现内存超限，应拆分为专项模板而不是压缩结论。 |
...
```bash
python3 scripts/ui/generate_algo_template_variants.py --out-dir assets/templates
python3 -m json.tool assets/templates/template_manifest.json >/tmp/template_manifest.check.json
python3 - <<'PYX'
from pathlib import Path
import openpyxl
for p in sorted(Path("assets/templates").glob("algo_*.xlsx")):
    wb = openpyxl.load_workbook(p, read_only=True, data_only=True)
    assert "词条预处理" in wb.sheetnames, p
PYX
```
