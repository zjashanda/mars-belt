...
3021 后续烧录、验证、复测只保留这一套控制逻辑：

- `uut-switch1` 只控制设备电源。
- `uut-switch2` 只控制协议口连接门控；任何断电/上电动作前先 `uut-switch2.off`，上电稳定后再 `uut-switch2.on`。
- `uut-switch3` 是唯一 boot 控制线。
- 历史调试中出现过的其他 switch 组合、把协议门控当 boot 线、不带协议门控的直接电源序列均已废弃，不得作为后续验证或脚本默认逻辑。
