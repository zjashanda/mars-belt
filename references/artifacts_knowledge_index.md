...
| 3021 设备 bring-up 与串口/电源定位 | `artifacts/runtime/device-baseline/*`、`artifacts/platform-validation/manual-port-capture-*`、`artifacts/device-check/*` | ACM0/ACM1/ACM2 对照、协议口拔插、历史控制实验、重复烧录稳定性。历史实验只作为问题来源追溯，不作为后续控制方案。 | 当前唯一 3021 口径：ACM0 日志/烧录、ACM2 协议、ACM4 控制；switch1 电源、switch2 协议门控、switch3 boot；任何断电/上电前先断协议口，上电稳定后恢复协议口。 | `references/3021_known_good_smoke_firmware.md`、`SKILL.md` 严格规则 17/18 |
